...
**Métodos clave:** TODOS — prioriza el bucle principal y olvídate del resto si el tiempo aprieta (BuscaPatron y Menu los últimos, Lee si tienes tiempo)

CON DIFERENCIA el examen más jodido de toda la historia de FP1 en esta carrera:

- Tienes que declarar un `Inicializa` con variables que le preguntamos al jugador, que no vienen dadas por un ejemplo.
...
- No hay `LeeInput` ni `ProcesaInput` — puntos y tiempo regalados que aquí no existen.
...
- Lo más complicado: `BuscaPatron`, que necesita 4 fors anidados y una iluminación divina si no sabes plantear la búsqueda de un array bidimensional dentro de otro.
